function orderShippedEvent(order, email, text) {
  ...
  cancelCalls(patientLabel, ['Order Shipped', 'Order Failed'])
...
function refillReminderEvent(order, email, text, hoursToWait, hourOfDay) {
  ...
  cancelCalls(patientLabel, ['Refill Reminder'])
...
function autopayReminderEvent(order, email, text, hoursToWait, hourOfDay) {
  ...
  cancelCalls(patientLabel, ['Autopay Reminder'])
...
function orderUpdatedEvent(order, email, text, hoursToWait) {
  ...
  cancelCalls(patientLabel, ['Order Updated'])
...
function orderFailedEvent(order, email, text, hoursToWait, hourOfDay) {
  ...
  cancelCalls(patientLabel, ['Order Failed'])
...
function newEvent(eventTitle, commArr, hoursToWait, hourOfDay) {
  ...
  var eventStart = addMinutes(hoursToWait || 0)

  if (hourOfDay)
    eventStart   = setHours(hourOfDay, eventStart)

  var eventStop  = addMinutes(30, eventStart) //Arbitrary length of 30mins so we can see it on calendar
...
function searchEvents(patientLabel, typeArr) {
  ...
  var calendar = CalendarApp.getCalendarById(GCALENDAR_ID)
...
function removeDrugsFromEvents(patientLabel, drugs, typeArr) {
  ...
  var log   = []
  var calls = searchCalls(patientLabel, typeArr)

  for (var i in calls) {
      var oldCall = calls[i].getDescription() //This is still JSON.stringified
    var regex   = new RegExp('('+drugs.join('|')+')[^;]*;', 'g')
    var newCall = oldCall.replace(regex, '')

    if (oldCall == newCall) continue

    if ( ~ newCall.indexOf(';')) {
      log.push(['modified an event', eventString(calls[i]), newCall])
      calls[i].setDescription(newCall)
    }
    else {
      log.push(['deleted an event', eventString(calls[i])])
      calls[i].deleteEvent()
    }
  }
