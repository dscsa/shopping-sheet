...
function _setV2info(drug) {
  ...
  if ( ! drug.$Stock && v2info['order.maxInventory'] && (v2info['order.maxInventory'] > 3500) && (v2info["price / month"] < 20) && (v2info['order.maxInventory']/v2info['qty threshold'] > 10))
    sendEmail('Change Order for High stock Item', ['Consider updating v2 order for '+drug.$v2, drug.$v2+' ('+drug.$Name+') is high stock but its max inventory of '+v2info['order.maxInventory']+' is already way higher than its qty threshold of '+v2info['qty threshold']+' '+emailJson])
  else if ( ! drug.$Stock && (v2info['inventory.qty'] > 1000) && (v2info["price / month"] < 20) && (v2info['inventory.qty']/v2info['qty threshold'] > 10) && (v2info['order.minQty'] < 5 || v2info['order.minDays'] < 150))
