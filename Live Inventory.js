...
function _setV2info(drug) {
  ...
  if ( ! drug.$Stock && v2info['order.maxInventory'] && (v2info['order.maxInventory'] > 3500) && (v2info["price / month"] < 20) && (v2info['order.maxInventory'] > v2info['dispensed.qty']+3*v2info['qty threshold']))
    sendEmail('Change Order for High stock Item', ['Consider updating v2 max inventory for '+drug.$v2, drug.$v2+' ('+drug.$Name+') '+v2info['order.maxInventory']+' > '+v2info['dispensed.qty']+' + 3*'+v2info['qty threshold']+' '+emailJson])
    ...
    sendEmail('Change Order for High stock Item', ['Consider increasing v2 minDays & minQty for '+drug.$v2, drug.$v2+' ('+drug.$Name+') '+v2info['inventory.qty']+' > '+v2info['dispensed.qty']+' + 3*'+v2info['qty threshold']+' '+emailJson])
