...
function _setV2info(drug) {
  ...
  if ( ! drug.$Stock && v2info['order.maxInventory'] && (v2info['order.maxInventory'] > 3500) && (v2info["price / month"] < 20) && (v2info['order.maxInventory'] > v2info['dispensed.qty']+v2info['qty threshold']*3))
    sendEmail('Change Order for High stock Item', ['Consider updating v2 order for '+drug.$v2, drug.$v2+' ('+drug.$Name+') is high stock but its max inventory of '+v2info['order.maxInventory']+' is already way higher than its qty threshold of '+v2info['qty threshold']+' and its dispensed.qty of '+v2info['dispensed.qty'] '+emailJson])
  else if ( ! drug.$Stock && (v2info['inventory.qty'] > 1000) && (v2info["price / month"] < 20) && (v2info['inventory.qty'] > v2info['dispensed.qty']+v2info['qty threshold']*3) && (v2info['order.minQty'] < 5 || v2info['order.minDays'] < 150))
    sendEmail('Change Order for High stock Item', ['Consider updating v2 order for '+drug.$v2, drug.$v2+' ('+drug.$Name+') is high stock with a inventory of '+v2info['inventory.qty']+'but is ordered even if it has a quantity > '+v2info['order.minQty']+' and expires more than '+v2info['order.minDays']+' days from now'+' '+emailJson])
