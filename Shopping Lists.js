...
function createShoppingLists(order, drugs) {
  ...
  var ss   = openSpreadsheet('Shopping List #'+orderID, 'Shopping Lists v4')
  var errs = []
  for (var i in drugs) {
    try {
  ...
  function createShoppingList(drug, order) {
    ...
    try {
      ...
      var vals = [
        ['Order #'+orderID+' '+(+i+1)+' of '+(drugs.length-1)+': '+drug.$Name+' '+(new Date().toJSON()), '', '' ,'', ''],
        ['Days:'+minDays+', Qty:'+minQty+', Count:'+shopped.list.length+(drug.$Stock ? ' ('+drug.$Stock+')' : ''), '', '', '', ''],
        ['', '', '', '', '']
      ].concat(shopped.list)

      ss.getRange('A1:E'+vals.length).setValues(vals).setHorizontalAlignment('left')
...
function testSheet() {
  var ss = openSpreadsheet('Shopping List #ADAM', 'Shopping Lists v4')
